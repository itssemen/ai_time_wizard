...
PLACEHOLDERS = {
    "кому": ["маме", "другу", "коллеге", "начальнику", "клиенту", "подруге", "брату", "партнеру", "поставщику", "HR", "бухгалтеру", "аналитику", "разработчику"],
    "от кого": ["банка", "партнера", "руководства", "клиента", "коллеги", "друга", "отдела маркетинга", "технической поддержки"],
    "что": ["этот вопрос", "детали проекта", "план работ", "условия договора", "презентацию", "ужин", "отчет", "концепцию", "макет", "прототип", "спецификацию", "требования"],
    "чем_покупки": ["хлебом", "молоком", "овощами", "фруктами", "необходимым", "бытовой химией", "канцтоварами", "кофе", "чаем", "вкусняшками к чаю"],
    "где_дом": ["комнате", "квартире", "на кухне", "в гостиной", "в ванной", "на балконе", "в кабинете", "в спальне", "в кладовке"],
...
HIGH_PRIORITY_KEYWORDS = ["срочно", "важно", "немедленно", "в первую очередь", "обязательно", "критично", "дедлайн", "горит", "кровь из носу", "экстренно", "безотлагательно"]
LOW_PRIORITY_KEYWORDS = ["если будет время", "потом", "не горит", "можно отложить", "второстепенно", "когда-нибудь", "не к спеху", "по возможности", "опционально", "позже"]
...
# Генерация датасета
NUM_EXAMPLES = 2500 # <<< ИЗМЕНЕНО здесь и ниже в сообщениях
dataset = []
generated_count = 0
attempt_limit = NUM_EXAMPLES * 4 # Увеличим лимит попыток еще больше, т.к. генерация может быть сложнее с большим разнообразием
