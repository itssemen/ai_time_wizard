...
TASK_CATEGORIES = {
    "короткие_коммуникации": {
        ...
        "duration_range": (3, 21), # Было (5, 30)
        "keywords_for_duration": {"позвонить": (3,14), "написать": (3,18), "проверить почту": (3,10), "согласовать": (7,21), "запросить фидбек": (5,15), "уведомить": (2,10)}
    },
    "быт_еда": {
        "actions": [
        ...
        "duration_range": (7, 63), # Было (10, 90)
        "keywords_for_duration": {"приготовить": (18,63), "сходить в магазин": (14,53), "заказать": (3,10), "кофе/чай": (2,7), "покормить": (3,10)}
    },
    "уборка_организация": {
        "actions": [
        ...
        "duration_range": (14, 105), # Было (20, 150)
        "keywords_for_duration": {"сделать уборку": (28,126), "разобрать": (21,84), "организовать файлы": (14,63), "выбросить": (5,20)}
    },
    "работа_проекты": {
        "actions": [
        ...
        "duration_range": (21, 210), # Было (30, 300)
        "keywords_for_duration": {"отчет": (42,210), "исследование": (63,252), "код": (32,168), "презентац": (42,168), "тестировать": (30,120), "деплой": (15,90), "тикет": (5,15)}
    },
    "встречи_созвоны": {
        "actions": [
        ...
        "duration_range": (10, 84), # Было (15, 120)
        "keywords_for_duration": {"совещание": (21,84), "планерка": (10,42), "one-on-one": (21,42), "синхронизироваться": (10,30), "демо": (20,60)}
    },
    "обучение_развитие": {
        "actions": [
        ...
        "duration_range": (21, 168), # Было (30, 240)
        "keywords_for_duration": {"изучить": (28,105), "курс": (42,168), "лекцию": (32,84), "статью": (15,60), "модуль": (30,90)}
    },
    "личные_дела_поручения": {
        ...
        "duration_range": (7, 84), # Было (10, 120)
        "keywords_for_duration": {"записаться": (3,21), "оплатить счета": (7,28), "подать документы": (21,63), "забронировать": (5,25), "продлить": (10,40)}
    },
    "отдых_хобби": {
        "actions": [
        ...
        "duration_range": (14, 168), # Было (20, 240)
        "keywords_for_duration": {"посмотреть фильм": (53,126), "погулять": (21,84), "поспать": (210, 420), "почитать книгу": (21,84), "серию сериала": (30,60), "полистать соцсеть": (10,40)}
    },
    "спорт_здоровье": {
        "actions": [
        ...
        "duration_range": (10, 105), # Было (15, 150)
        "keywords_for_duration": {"тренировка": (28,70), "пробежка": (18,63), "врача": (21,63), "массаж": (40,70), "прогуляться на свежем воздухе": (15,45)}
    },
    "творчество_создание": {
        "actions": [
        ...
        "duration_range": (32, 210), # Было (45, 300)
        "keywords_for_duration": {"написать текст": (42,168), "дизайн": (63,210), "видео": (84,252), "смонтировать ролик": (60,180), "написать пост": (20,70)}
    },
    "планирование_анализ_будущего": {
        ...
        "duration_range": (14, 84), # Было (20, 120)
        "keywords_for_duration": {"спланировать": (21,63), "проанализировать": (32,84), "подвести итоги": (15,45), "оценить риски": (20,60)}
    },
    "общие_задачи_с_новым_контекстом": { # Новая категория для использования ADDITIONAL_ACTIONS_CONTEXTS
        ...
        "duration_range": (7, 126), # Было (10, 180)
        "keywords_for_duration": {"проверить": (7,42), "подготовить": (21,84), "проанализировать": (32,126), "создать": (42,168), "тщательно": (0,0)} # 'тщательно' само по себе не задает время, но может влиять в комбинации
    },
    "поездки_транспорт": { # Новая категория
        ...
        "duration_range": (15, 180), # Диапазон для поездок, сильно зависит от контекста
        "keywords_for_duration": {"съездить": (20,120), "доехать": (15,90), "отвезти": (20,100), "забрать": (20,100)}
    },
    "финансы_бюджет": { # Новая категория
        ...
        "duration_range": (5, 60),
        "keywords_for_duration": {"проверить баланс": (2,10), "оплатить": (5,20), "перевести деньги": (5,15), "составить бюджет": (20,60), "проанализировать расходы": (15,45)}
    }
}
...
def get_realistic_duration(action_text_template, action_text_concrete):
    ...
    # Убедимся, что минимальная длительность хотя бы 5 минут, если не 0.
    if base_duration > 0 and base_duration < 5:
        return 5
...
def generate_task_item():
    ...
    # Временные указания и соответствующие минуты (уменьшены примерно на 30% и округлены до 5 мин)
    # Расширенный список, включающий более разнообразные и числовые варианты
    time_options = [
        ("часик", 40), ("пару часов", 85), ("полчаса", 20), ("часа полтора", 65), ("полтора часа", 65), # бывшие 60, 120, 30, 90, 90
        ("два часа", 85), ("три часа", 125), ("четыре часа", 170), ("пять часов", 210), ("шесть часов", 250), # бывшие 120, 180, 240, 300, 360
        ("минут 5", 5), ("минут 10", 10), ("минут 15", 15), ("минут 20", 20), ("минут 25", 25),
        ("минут 30", 30), ("минут 40", 40), ("минут 45", 45), ("минут 50", 50), ("минут 55", 55),
        ("минут 75", 50), ("минут 90", 65), # бывшие 75, 90 (округлены до ближайших 5)
        ("1 час", 40), ("2 часа", 85), ("3 часа", 125), ("4 часа", 170), ("5 часов", 210), # бывшие 60, 120, 180, 240, 300
        ("1.5 часа", 65), ("2.5 часа", 105), ("0.5 часа", 20), ("3.5 часа", 150), # бывшие 90, 150, 30, 210
        ("1ч", 40), ("2ч", 85), ("1ч30м", 65), ("45м", 45), ("90 мин", 65), ("15 мин", 15),
        ("120 минут", 85), ("180 минут", 125), # бывшие 120, 180
        ("на полчасика", 20), # было 30
        ("на часок-другой", random.choice([40, 65, 85, 105])), # бывшие [60, 90, 120, 150] -> ~[42, 63, 84, 105]
        ("на пару-тройку часов", random.choice([85, 105, 125, 150])),# бывшие [120,150,180,210] -> ~[84,105,126,147]
        ("немного времени на", random.choice([10, 15, 20, 30, 35])), # бывшие [15, 20, 30, 40, 50] -> ~[10,14,21,28,35]
        ("посвятить этому где-то", random.choice([30, 40, 50, 65, 70, 85, 105])), # бывшие [45,60,75,90,100,120,150] -> ~[31,42,52,63,70,84,105]
        ("займет примерно", random.choice([15, 20, 25, 30, 35, 50, 55, 70])), # бывшие [20,25,35,40,50,70,80,100] -> ~[14,17,24,28,35,49,56,70]
        ("буквально {N} минут", None), # Динамическое значение N (диапазон для N будет скорректирован ниже)
        ("около {N} часов", None), # Динамическое значение N (диапазон для N будет скорректирован ниже)
        ("примерно {N} мин.", None), # Динамическое значение N (диапазон для N будет скорректирован ниже)
        ("где-то {N} ч.", None), # Динамическое значение N (диапазон для N будет скорректирован ниже)
    ...
    if random.random() < 0.80: # Увеличена вероятность явного указания времени
        ...
        if "{N}" in duration_phrase_template:
            if "минут" in duration_phrase_template or "мин." in duration_phrase_template:
                # Используем уменьшенный диапазон для n_val
                n_val = random.choice([5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60]) # Было до 90
                actual_duration_minutes = n_val # n_val уже отражает целевую длительность
                ...
                # Используем уменьшенный диапазон для n_val
                n_val = random.choice([0.5, 1, 1.5, 2, 2.5, 3]) # Было до 4
                actual_duration_minutes = int(n_val * 60) # n_val уже отражает целевую длительность
